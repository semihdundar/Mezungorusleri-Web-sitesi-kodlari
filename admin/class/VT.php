...
class VT{
	...
	var $user="mezungor_db";
	var $password="semih199807";
	var $dbname="mezungor_db";
	var $baglanti;
	...
	public function VeriGetir2($alanlar="",$tablo="",$wherealanlar="",$wherearraydeger="",$ordeby="ORDER BY ID ASC",$limit="")
	{
		$this->baglanti->query("SET CHARACTER SET utf8");
		$sql="SELECT ".$alanlar." FROM ".$tablo; /*SELECT * FROM ayarlar*/
		if(!empty($wherealanlar) && !empty($wherearraydeger))
		{
			$sql.=" ".$wherealanlar; /*SELECT * FROM ayarlarWHERE */
			if(!empty($ordeby)){$sql.=" ".$ordeby;}
	...
	public function turkceMetin($val="")
	{
		$find = array('Ç', 'Ş', 'Ğ', 'Ü', 'İ', 'Ö', 'ç', 'ş', 'ğ', 'ü', 'ö', 'ı', '+', '#','?','*','!','.','(',')');
		$replace = array('c', 's', 'g', 'u', 'i', 'o', 'c', 's', 'g', 'u', 'o', 'i', 'plus', 'sharp','','','','','','');
		$string = strtolower(str_replace($find, $replace, $val));
  ...
  PRIMARY KEY (`ID`)
	...
	*/
	...
	public function IDGetir($tablo="")
	{
	 ...
	 public function MailGonder($mail=NULL,$konu=NULL,$mesaj=NULL)
	   {
				$posta = new PHPMailer();
			   	$posta->CharSet = "UTF-8";
				$posta->IsSMTP();                                   // send via SMTP
				$posta->Host     = "mail.mezungorusleri.com"; // SMTP servers
				$posta->SMTPAuth = true;     // turn on SMTP authentication
				$posta->Username = "bilgi@mezungorusleri.com";  // SMTP username
				$posta->Password = "9yz8apdzmz&T"; // SMTP password
				$posta->Port     = 587; 
				$posta->From     = "bilgi@mezungorusleri.com"; // smtp kullanýcý adýnýz ile ayný olmalý
				$posta->Fromname = "bilgi@mezungorusleri.com";
				$posta->AddAddress($mail, "bilgi@mezungorusleri.com");
				$posta->Subject  =  $konu;
				$posta->Body     =  $mesaj;
